...

{
...

    {
        ...
        return ServiceResult<IReadOnlyList<BranchDto>>.Success(
            ...
            AdminListQueryProfiles.ApplyToBranches(

                FilterAccessible(branches).Select(ToDto),
...

    {
        ...
        var existing = await _branchRepository.GetEntityByIdAsync(
...

    {
        ...
        var existing = await _branchRepository.GetEntityByIdAsync((int)request.Id, cancellationToken);
    ...
    private IEnumerable<BranchRecord> FilterAccessible(IEnumerable<BranchRecord> branches)
    ...
    private void EnsureBranchAccess(BranchRecord branch)
    ...
    private static BranchDto ToDto(BranchRecord entity) => new()
